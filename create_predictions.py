...
class FighterMatchupPredictor:
    ...
    def create_fighter_matchup(
        ...
        current_stats_a = self._calculate_fighter_stats(fighter_a, df, current_date, n_past_fights)
        current_stats_b = self._calculate_fighter_stats(fighter_b, df, current_date, n_past_fights)
        ...
        # Create current fight stats (now includes separate open/closing odds)
        ...
        output_filename = os.path.join(self.data_dir, 'matchup data/specific_matchup_data.csv')
    ...
    def _calculate_fighter_stats(
        ...
        loss_streak = fighter_recent_fights['loss_streak'].iloc[0] if 'loss_streak' in fighter_recent_fights.columns else 0
    ...
    def _process_elo_stats(self, current_fight: pd.Series) -> tuple:
    ...
    def _process_other_stats(self, current_fight: pd.Series) -> List:
        """Process additional fighter statistics."""
        ...
        return [
            ...
            win_streak_a - win_streak_b, self.utils.safe_divide(win_streak_a, win_streak_b),
            loss_streak_a, loss_streak_b,
            loss_streak_a - loss_streak_b, self.utils.safe_divide(loss_streak_a, loss_streak_b),
            exp_a, exp_b, exp_a - exp_b, self.utils.safe_divide(exp_a, exp_b),
            days_since_a, days_since_b,
            days_since_a - days_since_b, self.utils.safe_divide(days_since_a, days_since_b)
        ]
...
def main():
    ...
    # Create matchup data
    matchup_creator = UFCMatchupCreator()
