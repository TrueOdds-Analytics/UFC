...
class UFCScraper:
    def __init__(self):
        ...
        self.data = "C:/Users/William/PycharmProjects/Fight-Forecaster-AI/data"
    ...
    def update(self, fight_stats, fighter_stats):
        ...
        if os.path.exists(fight_stats):
            ...
            fight_cols = ["date", "fight_url", "event_url", "result", "last_5_comps_per_event", "fighter", "opponent",
                          "division", "method", "round", "time",
                          "fighter_url", "opponent_url", "referee", "time_format", "knockdowns", "sub_attempts",
                          "reversals", "control", "takedowns_landed",
            ...
            fighter_cols = ["name", "height", "reach", "stance", "dob", "url", "SLpM", "Str_Acc", "SApM", "Str_Def",
                            "TD_Avg",
        ...
        fight_stats_updated = fight_stats_updated.astype(
            {"knockdowns": np.int64, "sub_attempts": np.int64, "reversals": np.int64})
    ...
    def update_fighter_details(self, fighter_urls, saved_fighters):
        """
        Updates fighter attributes with new fighters not yet saved
        """
        ...
        for f_url in fighter_urls:
            if f_url in saved_fighter_urls:
                ...
                fighter_stats = soup.find('div',
                                          class_='b-list__info-box b-list__info-box_style_middle-width js-guide '
                                                 'clearfix').select(
                    'li')
                for x in fighter_stats:
                    line = x.text.split(":")
                    if len(line) > 1:  # prevents the /n/n/n/n lines from being read
    ...
    def get_fight_card(self, url):
        ...
        date = soup.select_one('li.b-list__box-list-item').text.strip().split('/n')[-1].strip()
        rows = soup.select('tr.b-fight-details__table-row')[1:]
        top5 = 1
        for row in rows:
            fight_det = {'date': [], 'fight_url': [], 'event_url': [], 'result': [], 'last_5_comps_per_event': [],
                         'fighter': [], 'opponent': [],
                         'division': [], 'method': [],
            ...
            for i in range(len(cols)):
                if i in {2, 3, 4, 5}:  # skip sub, td, pass, strikes
                    pass
    ...
    def get_fight_stats(self, url):
        ...
        if fight_details == None:
            ...
            sig_strike_details = soup.find('p', class_='b-fight-details__collapse-link_tot',
                                           string=re.compile('Significant Strikes')).find_next('tbody',
                                                                                               class_='b-fight-details__table-body')
            sig_columns = {'fighter': [], 'head_strikes': [], 'body_strikes': [], 'leg_strikes': [],
                           'distance_strikes': [],
...
if __name__ == "__main__":
    ...
    # Scrape and update fight data
    fighter_stats, fight_stats = scraper.scrape_fights()
    print("Fighter stats updated.")
    print("Fight stats updated.")

    # Print fighter details
    print("Fighter Details:")
    for _, fighter in fighter_stats.iterrows():
        print(f"Name: {fighter['name']}")
        print(f"Height: {fighter['height']}")
        print(f"Reach: {fighter['reach']}")
        print(f"Stance: {fighter['stance']}")
        print(f"DOB: {fighter['dob']}")
        print("---")

    # Get all fight stats
    all_fight_stats = scraper.get_all_fight_stats()
    print("All fight stats scraped.")
